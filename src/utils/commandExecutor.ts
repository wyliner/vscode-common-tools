...
export class CommandExecutor {
     ...
     * @param inputText 可选的输入文本，用于转换模式
     * @returns 执行结果的Promise
     */
    public static async execute(command: CommonCommand, inputText?: string): Promise<void> {
        // 记录命令执行
        ...
        try {
            if (command.operationMode === CommandOperationMode.TRANSFORM && !inputText) {
                // 转换模式需要输入，但没有提供
            ...
            const functionResult = command.fn(inputText);
